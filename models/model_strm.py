...
class DistanceLoss(nn.Module):
    ...
    def forward(self, support_set, support_labels, queries):
        ...
        query_embed = self.clsW(queries.view(-1, self.cfg.trans_linear_in_dim*self.temporal_set_size)) # 560[20x28] x 1024
        ...
        '''
        dist_all = torch.zeros(n_queries, self.cfg.TRAIN.WAY) # 20 x 5
        ...
        for label_idx, c in enumerate(unique_labels):
            ...
            class_k = class_k.view(-1, self.cfg.trans_linear_in_dim*self.temporal_set_size) # 140 x 4096
...
class TemporalCrossTransformer(nn.Module):
    ...
    def forward(self, support_set, support_labels, queries):
        ...
        '''
            4-queries * 5 classes x 5(5 classes) and store this in a logit vector
        '''
        all_distances_tensor = torch.zeros(n_queries, self.cfg.TRAIN.WAY) # 20 x 5
        ...
        for label_idx, c in enumerate(unique_labels):
            ...
            class_scores = torch.matmul(mh_queries_ks.unsqueeze(1), class_k.transpose(-2,-1)) / math.sqrt(self.cfg.trans_linear_out_dim) # 20 x 5 x 28 x 28
...
class CNN_STRM(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.new_dist_loss_post_pat = [DistanceLoss(cfg, s) for s in cfg.MODEL.TEMP_SET]

        # Linear-based patch-level attention over the 16 patches
        self.attn_pat = Self_Attn_Bot(self.cfg.trans_linear_in_dim, self.num_patches)
    ...
    def forward(self, context_images, context_labels, target_images):
        ...
        context_features = context_features.reshape(-1, self.cfg.trans_linear_in_dim, self.num_patches) # 200 x 2048 x 16
        target_features = target_features.reshape(-1, self.cfg.trans_linear_in_dim, self.num_patches) # 160 x 2048 x 16       
